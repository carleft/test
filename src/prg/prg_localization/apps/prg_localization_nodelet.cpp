...
#include <prg_localization/pose_estimator.hpp>

#include <fstream>
#include <cstring>


namespace prg_localization {

class PrgLocalizationNodelet : public nodelet::Nodelet {
public:
  using PointT = pcl::PointXYZI;

  PrgLocalizationNodelet() {
  }
  virtual ~PrgLocalizationNodelet() {
  }
  ...
  void onInit() override {
    ...
    if(use_imu) {
      ...
      imu_sub = mt_nh.subscribe("/gpsimu_driver/imu_data", 256, &PrgLocalizationNodelet::imu_callback, this);
    }
    points_sub = mt_nh.subscribe("/velodyne_points", 5, &PrgLocalizationNodelet::points_callback, this);
    globalmap_sub = nh.subscribe("/globalmap", 1, &PrgLocalizationNodelet::globalmap_callback, this);
    initialpose_sub = nh.subscribe("/initialpose", 8, &PrgLocalizationNodelet::initialpose_callback, this);
  ...
  void initialize_params() {
    ...
    registration = ndt;
      ...
      if(private_nh.param<bool>("specify_init_pose", true)) {
        ...
        pose_estimator.reset(new prg_localization::PoseEstimator(registration,
          ros::Time::now(),
          ...
          std::string file = "/home/tb/catkin_slam/src/prg_localization/data/pose.txt";
        std::ifstream infile; 
        ...
        if(infile)
        {
          ...
          while(infile>>x)             //遇到空白符结束
              {
                  *p=x;
                  p++;
              }
              NODELET_INFO("initialize pose from pose.txt :");
              for(int i = 0; i < 7; i ++)
              {
                  std::cout << init[i] << " ";
              }
            ...
            pose_estimator.reset(new prg_localization::PoseEstimator(registration,
            ros::Time::now(),
            Eigen::Vector3f(init[0], init[1], init[2]),
            Eigen::Quaternionf(init[3], init[4], init[5], init[6]),
            private_nh.param<double>("cool_time_duration", 0.5)
            ));
        }
      }
...
private:
  ...
  void initialpose_callback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& pose_msg) {
    ...
    pose_estimator.reset(
          new prg_localization::PoseEstimator(
            registration,
            ros::Time::now(),
...
private:
  ...
  std::unique_ptr<prg_localization::PoseEstimator> pose_estimator;
...
PLUGINLIB_EXPORT_CLASS(prg_localization::PrgLocalizationNodelet, nodelet::Nodelet)

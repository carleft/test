...
#include <prg_graph_slam/FloorCoeffs.h>

#include <prg_graph_slam/SaveMap.h>
#include <prg_graph_slam/DumpGraph.h>

#include <nodelet/nodelet.h>
#include <pluginlib/class_list_macros.h>

#include <prg_graph_slam/ros_utils.hpp>
#include <prg_graph_slam/ros_time_hash.hpp>

#include <prg_graph_slam/graph_slam.hpp>
#include <prg_graph_slam/keyframe.hpp>
#include <prg_graph_slam/keyframe_updater.hpp>
#include <prg_graph_slam/loop_detector.hpp>
#include <prg_graph_slam/information_matrix_calculator.hpp>
#include <prg_graph_slam/map_cloud_generator.hpp>
#include <prg_graph_slam/nmea_sentence_parser.hpp>
...
namespace prg_graph_slam {

class PrgGraphSlamNodelet : public nodelet::Nodelet {
public:
  typedef pcl::PointXYZI PointT;

  PrgGraphSlamNodelet() {}
  virtual ~PrgGraphSlamNodelet() {}
  ...
  virtual void onInit() {
    ...
    sync->registerCallback(boost::bind(&PrgGraphSlamNodelet::cloud_callback, this, _1, _2));
    imu_sub = nh.subscribe("/gpsimu_driver/imu_data", 1024, &PrgGraphSlamNodelet::imu_callback, this);
    floor_sub = nh.subscribe("/floor_detection/floor_coeffs", 1024, &PrgGraphSlamNodelet::floor_coeffs_callback, this);

    if(private_nh.param<bool>("enable_gps", true)) {
      gps_sub = mt_nh.subscribe("/gps/geopoint", 1024, &PrgGraphSlamNodelet::gps_callback, this);
      nmea_sub = mt_nh.subscribe("/gpsimu_driver/nmea_sentence", 1024, &PrgGraphSlamNodelet::nmea_callback, this);
      navsat_sub = mt_nh.subscribe("/gps/navsat", 1024, &PrgGraphSlamNodelet::navsat_callback, this);
    }

    // publishers
    markers_pub = mt_nh.advertise<visualization_msgs::MarkerArray>("/prg_graph_slam/markers", 16);
    odom2map_pub = mt_nh.advertise<geometry_msgs::TransformStamped>("/prg_graph_slam/odom2pub", 16);
    map_points_pub = mt_nh.advertise<sensor_msgs::PointCloud2>("/prg_graph_slam/map_points", 1);
    read_until_pub = mt_nh.advertise<std_msgs::Header>("/prg_graph_slam/read_until", 32);

    dump_service_server = mt_nh.advertiseService("/prg_graph_slam/dump", &PrgGraphSlamNodelet::dump_service, this);
    save_map_service_server = mt_nh.advertiseService("/prg_graph_slam/save_map", &PrgGraphSlamNodelet::save_map_service, this);

    double graph_update_interval = private_nh.param<double>("graph_update_interval", 3.0);
    double map_cloud_update_interval = private_nh.param<double>("map_cloud_update_interval", 10.0);
    optimization_timer = mt_nh.createWallTimer(ros::WallDuration(graph_update_interval), &PrgGraphSlamNodelet::optimization_timer_callback, this);
    map_publish_timer = mt_nh.createWallTimer(ros::WallDuration(map_cloud_update_interval), &PrgGraphSlamNodelet::map_points_publish_timer_callback, this);
  }
...
private:
   ...
   */
  void floor_coeffs_callback(const prg_graph_slam::FloorCoeffsConstPtr& floor_coeffs_msg) {
  ...
  bool flush_floor_queue() {
    ...
    auto remove_loc = std::upper_bound(floor_coeffs_queue.begin(), floor_coeffs_queue.end(), latest_keyframe_stamp,
      [=](const ros::Time& stamp, const prg_graph_slam::FloorCoeffsConstPtr& coeffs) {
   ...
   * @return
   */
  bool dump_service(prg_graph_slam::DumpGraphRequest& req, prg_graph_slam::DumpGraphResponse& res) {
    std::lock_guard<std::mutex> lock(main_thread_mutex);
   ...
   * @return
   */
  bool save_map_service(prg_graph_slam::SaveMapRequest& req, prg_graph_slam::SaveMapResponse& res) {
    std::vector<KeyFrameSnapshot::Ptr> snapshot;
...
private:
  ...
  std::deque<prg_graph_slam::FloorCoeffsConstPtr> floor_coeffs_queue;
...
PLUGINLIB_EXPORT_CLASS(prg_graph_slam::PrgGraphSlamNodelet, nodelet::Nodelet)

...
#include <prg_graph_slam/registrations.hpp>
...
private:
  ...
  void publish_odometry(const ros::Time& stamp, const std::string& base_frame_id, const Eigen::Matrix4f& pose) {
    ...
    p[3] = odom.pose.pose.orientation.w;
    p[4] = odom.pose.pose.orientation.x;
    p[5] = odom.pose.pose.orientation.y;
    p[6] = odom.pose.pose.orientation.z;
    /*
    std::cout << "x = " << odom.pose.pose.position.x << '\n'
              ...
              << "z = " << odom.pose.pose.orientation.z << std::endl;
    */
  ...
  void write()
  {
    //std::cout << "save!" << std::endl;
      ...
      outFile.open("/home/tb/catkin_slam/src/prg_localization/data/pose.txt");
      outFile << p[0] << '\n'
              << p[1] << '\n'
              << p[2] << '\n'
              << p[3] << '\n'
  ...
  void write1()
  {
    //std::cout << "save!" << std::endl;
      std::ofstream outFile;
      outFile.open("/home/tb/data.txt",std::ios::app);
      outFile 
              << p[0] << '\t'
              << p[1] << '\t'
              << p[2] << '\t'
...
PLUGINLIB_EXPORT_CLASS(prg_graph_slam::ScanMatchingOdometryNodelet, nodelet::Nodelet)
